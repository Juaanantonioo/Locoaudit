...
if False:  # dead code — kept only so the file diff stays readable
    def _old_run_gui(): pass

    # ── Window ──────────────────────────────────
    root = tk.Tk()
    root.title("LoCoAudit — Configuración")
    root.configure(bg="#1e1e2e")
    root.resizable(False, False)
    root.update_idletasks()
    sw = root.winfo_screenwidth()
    sh = root.winfo_screenheight()
    root.geometry(f"700x520+{(sw - 700) // 2}+{(sh - 520) // 2}")

    # ── Header ──────────────────────────────────
    tk.Label(
        root,
        text=f"LoCoAudit  v{VERSION}",
    ...
    # ── Step progress bar ────────────────────────
    step_frame = tk.Frame(root, bg="#1e1e2e")
    step_frame.pack(fill="x", padx=20, pady=(0, 6))

    _step_labels = []
    for txt in ("1 · Dependencias", "2 · Opcionales", "3 · Registrar",
                "4 · Dashboard", "5 · Flujos", "6 · Arrancar"):
        lbl = tk.Label(
            step_frame, text=txt,
            font=("TkDefaultFont", 9),
            fg="#6c7086", bg="#1e1e2e", padx=2,
        )
        lbl.pack(side="left", expand=True)
        _step_labels.append(lbl)

    # ── Log area ─────────────────────────────────
    log_outer = tk.Frame(root, bg="#181825", bd=1, relief="flat")
    log_outer.pack(fill="both", expand=True, padx=14, pady=(0, 6))

    _scrollbar = tk.Scrollbar(log_outer)
    ...
    _log.pack(fill="both", expand=True)
    _scrollbar.config(command=_log.yview)

    _log.tag_config("ok",   foreground="#a6e3a1")
    _log.tag_config("err",  foreground="#f38ba8")
    _log.tag_config("warn", foreground="#f9e2af")
    _log.tag_config("info", foreground="#89b4fa")
    _log.tag_config("dim",  foreground="#6c7086")

    # ── Final button (packed first so ask_frame can go before it) ──
    _btn_open = tk.Button(
        root,
        text="Abriendo Node-RED…",
        bg="#45475a", fg="#6c7086",
        font=("TkDefaultFont", 11, "bold"),
        relief="flat", padx=20, pady=8,
        state="disabled",
    )
    _btn_open.pack(pady=(0, 12), padx=14, fill="x")

    # ── Question area (hidden until an "ask" message arrives) ───
    _ask_frame = tk.Frame(root, bg="#313244", pady=8)
    _ask_lbl   = tk.Label(
        _ask_frame, text="",
        bg="#313244", fg="#cdd6f4",
    ...
    _ask_lbl.pack(pady=(4, 6))

    _ask_btn_row = tk.Frame(_ask_frame, bg="#313244")
    _ask_btn_row.pack()

    _ask_event_ref  = [None]   # type: list
    _ask_result_ref = [None]   # type: list

    def _on_yes():
        lst = _ask_result_ref[0]
        if lst is not None:
            lst.append(True)
        evt = _ask_event_ref[0]
        if evt is not None:
            evt.set()
        _ask_frame.pack_forget()
        ...
        try:
            while True:
                ...
                if mtype == "log":
                    text = msg.get("text", "")
                    tag  = msg.get("tag", "")
                    _log.config(state="normal")
                    _log.insert("end", text + "\n", (tag,) if tag else ())
                    _log.see("end")
                    _log.config(state="disabled")

                elif mtype == "progress":
                    step = msg.get("step", 1) - 1  # convert to 0-indexed
                    for i, lbl in enumerate(_step_labels):
                        if i < step:
                            lbl.config(fg="#a6e3a1")  # completed
                        elif i == step:
                            lbl.config(fg="#89b4fa")  # current
                        else:
                            lbl.config(fg="#6c7086")  # pending

                elif mtype == "ask":
                    _ask_event_ref[0]  = msg["event"]
                    _ask_result_ref[0] = msg["result"]
                    lbl_text = msg.get("label", msg.get("tool", "?"))
                    cmd_text = msg.get("cmd", "")
                    _ask_lbl.config(
                        text=f"¿Deseas instalar {lbl_text} ahora?\n↳ {cmd_text}"
                    )
                    _ask_frame.pack(
                        fill="x", padx=14, pady=(0, 6), before=_btn_open
                    )
