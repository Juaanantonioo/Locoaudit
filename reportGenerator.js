...
function severityColor(s) {
  return { CRITICAL: "#c0392b", HIGH: "#e74c3c", MEDIUM: "#e67e22", LOW: "#f1c40f", INFO: "#2ecc71" }[s] || "#95a5a6";
}

function severityBadge(s) {
  const colors = { CRITICAL: "#c0392b", HIGH: "#e74c3c", MEDIUM: "#e67e22", LOW: "#f1c40f", INFO: "#27ae60" };
...
function maxSeverityColor(s) {
  return severityColor(s);
}

// ── Texto amigable para usuarios no técnicos ────────────────────────────────

const PLAIN_MESSAGES = {
  MEMORY_FREE_RATIO: {
    HIGH:   "Tu ordenador tiene muy poca memoria libre. Puede que vaya lento o falle pronto.",
    MEDIUM: "La memoria de tu ordenador está bastante llena. Cierra programas que no uses.",
    LOW:    "La memoria empieza a estar algo justa. Vigílala.",
    INFO:   "La memoria del ordenador está en buen estado.",
  },
  NO_EXTERNAL_IPV4:    "No se detectó ninguna conexión de red activa. Revisa el cable o el WiFi.",
  EXTERNAL_IPV4_DETECTED: "El ordenador tiene conexión a la red. Todo parece normal.",
  NO_OPEN_PORTS_DETECTED: "No se encontraron puertas de acceso abiertas. Eso es bueno.",
  OPEN_PORTS_DETECTED: "Se encontraron accesos activos en la red. Revisa si todos son necesarios.",
  EXPOSURE_CONTEXT: {
    HIGH:   "Hay accesos peligrosos abiertos que cualquiera en la red podría usar para entrar a tu equipo. Actúa pronto.",
    MEDIUM: "Algunos accesos están abiertos en tu red local. No es urgente, pero conviene revisarlos.",
  ...
  NODE_RED_PORT_1880_OPEN: {
    ...
    MEDIUM: "Node-RED está abierto a la red. Asegúrate de que tenga contraseña.",
  },
  NMAP_NOT_AVAILABLE:       "La herramienta de escaneo de red (Nmap) no está instalada. El análisis de puertos no pudo completarse.",
  TRIVY_NOT_AVAILABLE:      "La herramienta de análisis de vulnerabilidades (Trivy) no está instalada. No se pudo analizar el software.",
  NO_VULNERABILITIES_DETECTED: "No se encontraron vulnerabilidades conocidas en el software instalado. ¡Bien!",
  HTTPFP_NO_ENDPOINTS:      "No se encontraron servicios web activos para analizar.",
};
...
function getPlainMessage(finding) {
  ...
  if (typeof entry === "object") return entry[finding.severity] || entry["INFO"] || null;
  return null;
}

// ── Generador de sección de un finding ─────────────────────────────────────
...
// ── Render de una auditoría individual ─────────────────────────────────────
...
function renderAuditPanel(report, auditIndex) {
  ...
  <div style="padding:20px;">
    ...
    <!-- Resumen ejecutivo -->
    <div style="background:linear-gradient(135deg,${maxSeverityColor(max)}22,#fff);border:1px solid ${maxSeverityColor(max)}55;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
...
function renderDashboard(reports) {
  // Agrupar por hostname (o deviceLabel si existe)
  const devices = {};
  ...
  // Ordenar auditorías de cada dispositivo de más reciente a más antigua
  ...
  const sidebarHtml = deviceKeys.map((key, di) => {
    const latest = devices[key][0];
    const max = latest?.summary?.maxSeverity || "INFO";
    const label = latest?.deviceLabel || key;
    const lastScan = latest?.scanId ? new Date(latest.scanId).toLocaleString("es-ES") : "—";

    return `
    <div class="device-item ${di === 0 ? "active" : ""}"
         onclick="selectDevice(${di})"
         id="device-btn-${di}"
         style="padding:12px 16px;cursor:pointer;border-left:4px solid ${di === 0 ? maxSeverityColor(max) : "transparent"};
  ...
  // Panel principal: tabs de historial por dispositivo
  const mainPanelsHtml = deviceKeys.map((key, di) => {
    const audits = devices[key];

    const tabsHtml = audits.map((r, ai) => {
      const label = r.scanId ? new Date(r.scanId).toLocaleString("es-ES") : `Auditoría ${ai + 1}`;
      ...
      <button onclick="selectAudit(${di},${ai})"
              ...
              style="padding:6px 12px;border:none;border-radius:20px;cursor:pointer;font-size:0.78rem;font-weight:600;
                     background:${ai === 0 ? maxSeverityColor(max) : "#ecf0f1"};
                     color:${ai === 0 ? "#fff" : "#555"};
    ...
    const auditContentsHtml = audits.map((r, ai) => `
...
<script>
  ...
  function selectAudit(di, ai) {
    // Ocultar contenidos del dispositivo
    var idx = 0;
    while (true) {
 ...
 * @param {string} baseDir - Directorio donde están los JSON y donde se escribe el HTML.
...
function generateDashboard(baseDir) {
  ...
  for (const file of files) {
    ...
    } catch (_) { /* skip ficheros corruptos */ }
  }
