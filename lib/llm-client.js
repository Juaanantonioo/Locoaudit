...
function buildRulesBlock(auditType, platform) {
  ...
  if (auditType === "host" || auditType === "network") {
  ...
  if (auditType === "image") {
    ...
    lines.push(
      ...
      "- DISTINGUE SIEMPRE dos acciones diferentes y dilo explícitamente:",
      "  · CERRAR el puerto = detener el servicio que escucha (systemctl stop <servicio> y su .socket en Linux,",
      "    launchctl en macOS). El puerto deja de estar abierto.",
      "  · FILTRAR con cortafuegos = el servicio sigue activo y el puerto sigue abierto; solo se bloquea el",
      "    tráfico que llega desde la red. Una regla de firewall NO cierra el puerto.",
