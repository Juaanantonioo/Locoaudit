...
const FIXES = {
  ...
  telnet:       (p) => removeFix("telnet", p),
  "rsh-server": (p) => removeFix("rsh-server", p),
  "rsh-client": (p) => removeFix("rsh-client", p),
  rlogin:       (p) => removeFix("rsh-client", p),
  tftpd:        (p) => removeFix("tftpd", p),
  "tftpd-hpa":  (p) => removeFix("tftpd-hpa", p),
  nis:          (p) => removeFix("nis", p),
  "yp-tools":   (p) => removeFix("yp-tools", p),
  talk:         (p) => removeFix("talk", p),
  ntalk:        (p) => removeFix("ntalk", p),
};
 ...
 * @returns {string}
 */
function removeFix(pkg, platform) {
  if (platform === "linux") {
    return `sudo apt remove --purge ${pkg}\n(en Fedora/RHEL: sudo dnf remove ${pkg})`;
  }
  return `Desinstala ${pkg} si no lo necesitas: es un servicio/protocolo heredado sin cifrado. ` +
    `Usa el gestor de paquetes de tu sistema para eliminarlo.`;
}
