 ...
 * @param {object} [opts]       Opciones extra. maxBuffer (bytes) para salidas
 *                              grandes: `trivy image` sobre imágenes voluminosas
 *                              (modelos de IA, etc.) puede emitir >10 MiB de JSON
 *                              y reventaría el buffer por defecto.
 * @returns {Promise<string>}   stdout del proceso.
 */
function execCommand(cmd, timeoutMs = 10000, opts = {}) {
  return new Promise((resolve, reject) => {
    const child = exec(
      cmd,
      {
        timeout: timeoutMs,
        maxBuffer: opts.maxBuffer || 10 * 1024 * 1024, // 10 MiB por defecto
