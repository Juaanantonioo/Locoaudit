  ...
  if (!win.securityLogDenied) {
    findings.push(
      createFinding({
        ...
        // El comando va SOLO en la primera línea: es lo que copia el dashboard
        // (campo `command`). Las aclaraciones van debajo, donde el botón
        // «Copiar» no las alcanza — pegar el paréntesis rompía la sintaxis.
        fix:
          'auditpol /set /subcategory:"Creación del proceso" /success:enable /failure:enable\n' +
          "Ejecútalo en una terminal abierta como administrador. " +
          'Si tu Windows está en inglés, la subcategoría se llama "Process Creation".',
        command:
          'auditpol /set /subcategory:"Creación del proceso" /success:enable /failure:enable',
        category: "system-logs",
        source: "security-events",
