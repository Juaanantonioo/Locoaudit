...
function fromTrivyJson(raw, idPrefix, category = "vulnerability", source = "trivy", opts = {}) {
  ...
  for (const result of results) {
    ...
    for (const v of (result.Vulnerabilities || [])) {
      ...
      const originKey = v.PkgPath ? "installed" : "declared";
      ...
      // Procedencia, con UNA sola señal explícita del JSON de Trivy: PkgPath.
      // Presente ⇒ hay un paquete real en disco. Ausente ⇒ Trivy leyó una
      // anotación en un fichero de bloqueo. No se deduce de la ruta ni del Class.
      const origin   = v.PkgPath ? "installed" : "declared";
