...
function fromLynisRaw(raw, platform, pkgManager) {
  ...
  for (const [i, w] of individualWarnings.entries()) {
    ...
    const lynFix = getLynisFixText(w.id, platform, pkgManager);
    findings.push(
      createFinding({
        ...
        fix: lynFix.fix,
        // command explícito: el generador sabe si el paso es un comando copiable.
        command: lynFix.command,
        category: "system",
        source: "lynis",
      })
    );
...
/**
 * FALLBACK — determina si un texto de fix "parece" un comando ejecutable.
 *
 * NO es la fuente de verdad de isCommand. La decisión la toma ahora el generador
 * del fix (getTrivyFixCommand / getLynisFixText / pkg-manager), que devuelve la
 * línea copiable en el campo `command` y la pasa explícita a createFinding. Este
 * helper solo queda como red de seguridad para llamadas legacy que aún pasan un
 * `fix` suelto sin `command`. No añadas lógica de gestores nuevos aquí.
 *
 * @param {string|null} fix
 * @returns {boolean}
 */
function isExecutableCommand(fix) {
  // Mismo predicado de fallback que usa createFinding (finding-schema::_isCmd).
...
function fromTrivyJson(raw, idPrefix, category = "vulnerability", source = "trivy", opts = {}) {
  ...
  for (const [, grp] of pkgGroups) {
    ...
    if (context === "image") {
      ...
      const r = getTrivyFixCommand(target, first.PkgName, fixedVersion, platform, type, pkgManager);
      fixText = r.fix;
