 ...
 *
 * La plataforma tiene prioridad sobre el contenido del Target porque en macOS
 * los ficheros yarn.lock/package-lock.json encontrados en rutas de Homebrew
 * Cellar son dependencias internas de las herramientas instaladas con brew,
 * no proyectos del usuario. El comando correcto en macOS siempre es
 * "brew upgrade <paquete>", salvo que el Target apunte a un proyecto personal
 * del usuario (ruta bajo /Users/<user>/ fuera de /Cellar/).
 *
 * @param {string} target         Target de Trivy (ruta del fichero o directorio escaneado).
 ...
 * @returns {string|null}         Comando de actualización listo para copiar-pegar, o null.
 */
function getTrivyFixCommand(target, pkgName, fixedVersion, platform) {
  ...
  // ── PRIORIDAD 1: macOS ──────────────────────────────────────────────────────
  // brew es el gestor de sistema en macOS. Los lockfiles dentro de /Cellar/ son
  // dependencias internas de herramientas brew, no proyectos del usuario.
  // Excepción: lockfile en ruta personal del usuario fuera de /Cellar/.
  if (platform === "darwin") {
    const isUserProject = t.includes("/users/") && !t.includes("/cellar/") &&
      (t.includes("package-lock.json") || t.includes("yarn.lock"));
    if (isUserProject) {
      if (t.includes("yarn.lock"))        return `yarn upgrade ${pkg}${ver ? "@" + ver : ""}`;
      if (t.includes("package-lock.json")) return `npm update ${pkg}${ver ? "@" + ver : ""}`;
    }
  ...
  // ── PRIORIDAD 2: Linux ──────────────────────────────────────────────────────
  if (platform === "linux") {
    if (t.includes("/cellar/") || t.includes("/homebrew/"))
      return `brew upgrade ${pkg}`;
    if (t.includes("yarn.lock"))
      return `yarn upgrade ${pkg}${ver ? "@" + ver : ""}`;
    if (t.includes("package-lock.json") || t.includes("node_modules"))
      return `npm update ${pkg}${ver ? "@" + ver : ""}`;
    if (t.includes("composer.lock") || t.includes("composer.json"))
      return `composer update ${pkg}`;
    if (t.includes("go.mod") || t.includes("go.sum"))
      return `go get ${pkg}@${ver || "latest"}`;
    if (t.includes("requirements.txt") || t.includes("pyproject.toml"))
      return `pip install --upgrade ${pkg}${ver ? "==" + ver : ""}`;
    if (t.includes("/var/lib/rpm") || t.includes("/usr/lib64"))
      return `sudo dnf update ${pkg}`;
    // Fallback Linux: apt (cubre dpkg, /usr/lib, /usr/share y caso genérico)
  ...
  for (const [, grp] of pkgGroups) {
    ...
    findings.push(
      createFinding({
        ...
        evidence,
        fix:      fixedVersion
          ? getTrivyFixCommand(target, first.PkgName, fixedVersion, process.platform)
...
function fromNmap(raw) {
  ...
  return raw
    ...
    .map((p, i) => {
      ...
      // Título: incluye el proceso cuando service-detect lo haya enriquecido
      const title = p.process
        ? `Puerto abierto: ${p.port}/tcp (${service}) — usado por ${p.process}`
        : `Puerto abierto: ${p.port}/tcp (${service})`;
...
function fromPortScanner(ports) {
  ...
  return ports.map((p, i) => {
    ...
    // Título: incluye el proceso si está disponible
    const title = p.process
      ? `Puerto abierto: ${p.port}/tcp (${p.service}) — usado por ${p.process}`
      : `Puerto abierto: ${p.port}/tcp (${p.service})`;

    // Evidence: acumula toda la información disponible
    let evidence = `Puerto ${p.port}/tcp abierto. Servicio: ${p.service}.`;
