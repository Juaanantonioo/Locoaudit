...
function buildImageFix({ image, pkg, fixedVersion } = {}) {
  ...
  const pkgFixed =
    pkg && fixedVersion
      ...
      : "";
  return (
    "Una imagen Docker es inmutable: sus paquetes NO se actualizan una vez construida. Opciones reales, en orden:\n" +
    `1. Cambia a un tag más reciente de ${img} que ya incluya la corrección. ${pkgFixed}` +
    "Comprueba en el registro (Docker Hub / GHCR) qué tag la trae.\n" +
    "2. Si ningún tag la corrige aún, cambia a una imagen base más pequeña y mantenida " +
    "(p.ej. alpine o distroless en vez de debian): menos paquetes = menos superficie de ataque.\n" +
    "3. Si el mantenedor aún no ha publicado corrección, no hay fix disponible: documenta " +
    "el riesgo aceptado y vigila las actualizaciones del upstream.\n" +
    "4. Si es una imagen propia, reconstrúyela sobre una base actualizada (FROM al día) y vuelve a escanear."
  );
}
