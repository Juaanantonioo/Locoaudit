...
/**
 * Clasifica el ratio de memoria usada.
 * @param {number} usedRatio  0..1
 * @returns {string}
 */
function memorySeverity(usedRatio) {
 ...
 * @param {{ cpu: Object, memory: Object }} raw
 * @returns {Finding[]}
 */
function fromCpuMemory(raw) {
  if (!raw) return [];
  const findings = [];
  ...
  // Memoria — severidad según ratio de uso
  if (raw.memory) {
    const { totalGiB, usedGiB, freeRatio } = raw.memory;
    const usedRatio = freeRatio != null ? 1 - freeRatio : null;
    const sev = usedRatio != null ? memorySeverity(usedRatio) : "info";
    const usedPct = usedRatio != null ? Math.round(usedRatio * 100) : "?";
 ...
 * @param {{ hardeningIndex, warnings, suggestions }} raw
 * @returns {Finding[]}
 */
function fromLynisRaw(raw) {
  ...
  // Advertencias (WARNING → high)
  for (const [i, w] of (raw.warnings || []).entries()) {
    findings.push(
      createFinding({
        id: `HOST-LYN-${w.id || counter(i)}`,
        title: `Lynis WARNING: ${w.id || "desconocido"}`,
        severity: fromLynis("WARNING"),
        evidence: w.description || w.id,
        fix: "Consultar la base de conocimiento de Lynis para este ID.",
        category: "system",
        source: "lynis",
      })
    );
  ...
  // Sugerencias (SUGGESTION → low)
  for (const [i, s] of (raw.suggestions || []).entries()) {
    findings.push(
      createFinding({
        id: `HOST-LYN-SUG-${counter(i)}`,
        title: `Lynis SUGGESTION: ${s.id || "desconocido"}`,
        severity: fromLynis("SUGGESTION"),
        evidence: s.description || s.id,
        fix: "Evaluar si aplicar esta sugerencia según el contexto del sistema.",
        category: "system",
        source: "lynis",
      })
    );
...
function normalizeHost(raw, source = "native") {
  ...
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    const findings = [];
    if (raw.cpuMemory) findings.push(...fromCpuMemory(raw.cpuMemory));
